...
void ADCDMA::start(size_t freqHZ) {
	...
	DMA_InitStructure.DMA_PeripheralDataSize = DMA_PeripheralDataSize_Word; // 8-bit data samples, Word????
	DMA_InitStructure.DMA_MemoryDataSize = DMA_MemoryDataSize_Word; // 8-bit data samples, 16-bit per access
	...
	ADC1_InitStructure.ADC_ExternalTrigConvEdge = ADC_ExternalTrigConvEdge_None;  //ADC_ExternalTrigConvEdge_Rising;
	...
	// This disables continuous conversion mode. When enabled, the ADC would continuously convert the input signal and provide a stream of digital data. Disabling it means that the ADC will perform a single conversion and stop.
	ADC2_InitStructure.ADC_ContinuousConvMode = ENABLE;//Use continuous mode for higher freq
	// This sets the ADC to start conversion at the rising edge of an external trigger
	...
	// No need to configure ADC2 for interleaved mode
    // ADC_RegularChannelConfig(ADC2, PIN_MAP[pin].adc_channel, 1, ADC_SampleTime_3Cycles);
